...
(function initSmartHeader(){
  ...
  // This header changes height between normal/compact.
  // If we toggle based on a small threshold, the height change itself can nudge scroll position
  // and cause rapid flip-flopping near the top.
  //
  // Fix:
  // - Enter compact as soon as the user scrolls down (y > 0).
  // - Exit compact ONLY after the page has been at the very top (y === 0) continuously for a short time.
  //   (This prevents a single layout nudge from triggering an immediate expand → re-compact loop.)
  ...
  const scheduleExitIfAtTop = ()=>{
    if (exitTimer) return;
    exitTimer = setTimeout(()=>{
      exitTimer = null;
      const y = Math.max(0, window.scrollY || 0);
      if (y === 0) apply(false);
    }, 120); // small delay to let layout settle
  };
