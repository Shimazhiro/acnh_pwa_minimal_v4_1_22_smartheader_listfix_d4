...
// smart header: shrink on scroll (mobile only; avoid jitter on PC)
(function initSmartHeader(){
  ...
  // NOTE:
  // At very small scroll positions, toggling header height can itself change scrollY a few pixels,
  // which can cause rapid on/off ("jitter"). We avoid that with hysteresis + mobile-only.
  ...
  const update = ()=>{
    // PCでは compact を完全無効化（高さ変化があると scrollY が揺れてループするため）
    if (!mqMobile.matches){
      apply(false);
      ticking = false;
      return;
  ...
  // 画面幅が変わった時（回転/リサイズ）も即座に整合させる
  if (mqMobile && typeof mqMobile.addEventListener === "function"){
    mqMobile.addEventListener("change", ()=> (window.requestAnimationFrame || setTimeout)(update, 0));
  } else if (mqMobile && typeof mqMobile.addListener === "function"){
    mqMobile.addListener(()=> (window.requestAnimationFrame || setTimeout)(update, 0));
  } else {
    window.addEventListener("resize", ()=> (window.requestAnimationFrame || setTimeout)(update, 0), {passive:true});
  }
